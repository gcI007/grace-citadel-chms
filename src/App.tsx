...
import { Users, ArrowLeft, Sparkles, Lock, LogOut, Cake, CheckSquare, Search, UserPlus, ClipboardList, UserMinus, BarChart3, ShieldCheck, MessageCircle, MessageSquare, Heart, Gift, TrendingUp, CalendarDays, Share2 } from 'lucide-react';
...
export default function App() {
  ...
  // --- BULK MESSAGE STATE ---
  const [bulkGroup, setBulkGroup] = useState('Student'); // NEW
  const [bulkText, setBulkText] = useState(TEMPLATES.bulk_student); // NEW
  ...
  // --- FORM STATES (Upgraded with Department) ---
  ...
  const [department, setDepartment] = useState('Student'); // NEW
  ...
  // --- 🔥 NEW: BULK MESSAGE ENGINE 🔥 ---
  const handleSendBulkMessage = async (type: 'whatsapp' | 'sms') => {
    setIsSubmitting(true);
    // 1. Fetch phone numbers belonging strictly to that demographic
    const { data: targets } = await supabase.from('members').select('phone_number, gender, status');
    
    if (!targets || targets.length === 0) {
        setIsSubmitting(false);
        return alert('No members found matching that demographic group.');
    }

    // Filter local array to match choice
    let filtered = [];
    ...
    if (phones.length === 0) {
        setIsSubmitting(false);
        return alert('No valid phone numbers found for this group.');
    }
    ...
    if (type === 'whatsapp') {
        // WhatsApp Bulk via pre-formed api linking line
        const phoneString = cleanPhones.join(',');
        window.open(`https://wa.me/?phone=${phoneString}&text=${encodedText}`, '_blank');
    } else {
        // SMS Group Text separation rule varies slightly based on platform: iOS uses comma, Android/Windows prefers semicolon
        const separator = navigator.userAgent.includes('iPhone') || navigator.userAgent.includes('iPad') ? ',' : ';';
        const phoneString = cleanPhones.join(separator);
        window.open(`sms:${phoneString}?body=${encodedText}`, '_blank');
    }
    setIsSubmitting(false);
  };
  ...
  const handleSaveMember = async (e: React.FormEvent) => {
    ...
    // If they register as a Student status, pass it gracefully
    try {
  ...
  const handleGroupChange = (group: string) => {
    ...
    if (group === 'Men') setBulkText(TEMPLATES.bulk_men);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 p-6">
        ...
        {activeView === 'dashboard' && (
          <div className="max-w-7xl mx-auto space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {isAdmin && (
                <>
                  <div className="bg-gradient-to-br from-gray-900 to-gray-800 p-6 rounded-xl shadow-md border border-gray-700 hover:shadow-lg transition-shadow"><h2 className="text-lg font-semibold mb-2 text-white flex items-center gap-2"><ShieldCheck size={18} className="text-orange-500"/> Pastoral Command Center</h2><button onClick={() => setActiveView('tasks')} className="text-orange-400 font-bold text-sm hover:underline">Open Command Center &rarr;</button></div>
                  
                  {/* NEW BULK OUTREACH NODE CARD */}
                  <div className="bg-gradient-to-br from-green-900 to-emerald-800 p-6 rounded-xl shadow-md border border-emerald-700 hover:shadow-lg transition-shadow">
                      <h2 className="text-lg font-semibold mb-2 text-white flex items-center gap-2"><Share2 size={18} className="text-green-400"/> Bulk Outreach Node</h2>
                      <p className="text-emerald-200 text-sm mb-4">Send instant broadcast messages to all Students, Men, or Women.</p>
                      <button onClick={() => setActiveView('bulk')} className="text-emerald-300 font-bold text-sm hover:underline">Open Bulk Engine 🚀 &rarr;</button>
                  </div>
        ...
        {/* --- 🔥 NEW: BULK OUTREACH INTERACTIVE ENGINE 🔥 --- */}
        {activeView === 'bulk' && isAdmin && (
            <div className="max-w-4xl mx-auto bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                ...
                <p className="text-gray-600 text-sm mb-6">Select a targeted group, verify your announcement script, and fire a broadcast linking directly to your device groups.</p>
                
                <div className="space-y-5 max-w-2xl">
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-2">Target Congregation Segment</label>
                        <div className="flex gap-4">
                            {['Student', 'Men', 'Women'].map(group => (
                                <label key={group} className="flex items-center gap-2 bg-gray-50 border px-4 py-3 rounded-md cursor-pointer select-none flex-1 hover:bg-gray-100 transition-colors">
                                    <input type="radio" checked={bulkGroup === group} onChange={() => handleGroupChange(group)} className="w-4 h-4 text-green-600" />
                                    <span className="font-semibold text-gray-800">{group} Only</span>
                                </label>
                            ))}
                        </div>
                    </div>
                    ...
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1">Message Body Announcement Script</label>
                        <textarea rows={5} value={bulkText} onChange={(e) => setBulkText(e.target.value)} className="w-full border border-gray-300 rounded-md p-3 font-medium outline-none focus:ring-2 focus:ring-green-500" placeholder="Type your broadcast announcement..." />
                    </div>

                    <div className="flex gap-4 pt-2">
                        <button onClick={() => handleSendBulkMessage('whatsapp')} className="flex-1 bg-green-600 text-white font-bold py-4 rounded-xl shadow hover:bg-green-700 flex items-center justify-center gap-2 transition-colors"><MessageCircle size={22}/> Broadcast via WhatsApp Group</button>
                        <button onClick={() => handleSendBulkMessage('sms')} className="flex-1 bg-blue-600 text-white font-bold py-4 rounded-xl shadow hover:bg-blue-700 flex items-center justify-center gap-2 transition-colors"><MessageSquare size={22}/> Broadcast via Bulk SMS</button>
                    </div>
                </div>
            </div>
        )}
          ...
          <div className="max-w-4xl mx-auto bg-white p-6 rounded-xl shadow-sm border">
            ...
            <form onSubmit={handleSaveMember} className="flex flex-col gap-4 max-w-md">
              <div className="bg-orange-50 p-4 rounded-lg border border-orange-100 mb-2">
                <label className="block text-sm font-bold text-orange-900 mb-1">Registration Type / Status Profile</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Guarded Birthday</label>
                  <div className="flex gap-2">
                    <select value={dobMonth} onChange={(e) => setDobMonth(e.target.value)} className="w-1/2 border border-gray-300 rounded-md p-2 bg-white"><option value="">Month</option>{['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'].map(m => <option key={m} value={m}>{m}</option>)}</select>
                    <select value={dobDay} onChange={(e) => setDobDay(e.target.value)} className="w-1/2 border border-gray-300 rounded-md p-2 bg-white"><option value="">Day</option>{[...Array(31)].map((_, i) => <option key={i+1} value={i+1}>{i+1}</option>)}</select>
                  </div>
                </div>
              </div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Occupation</label><input type="text" value={occupation} onChange={(e) => setOccupation(e.target.value)} className="w-full border border-gray-300 rounded-md p-2" placeholder="e.g. Student, Engineer" /></div>
              ...
              <button type="submit" className="bg-gray-900 text-white px-4 py-3 rounded-md font-medium">Save Registration</button>
              {statusMessage && <div className="p-3 bg-green-100 text-green-800 rounded-md text-sm">{statusMessage}</div>}
            </form>
          </div>
        )}
        ...
        {/* --- PREVIOUS SUB-VIEWS MINIMIZED FOR STABILITY --- */}
