...
export default function App() {
  ...
  const [template, setTemplate] = useState('welcome'); // NEW: Template State
  ...
  // --- MESSAGING HELPER ---
  const handleSendMessage = (phone: string | null, name: string, type: 'whatsapp' | 'sms') => {
    if (!phone) {
            ...
            {isAdmin && birthdays.length > 0 && (
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-4">
                ...
                <div className="w-full">
                  ...
                  <ul className="mt-2 text-sm text-blue-800 space-y-3">
                    {birthdays.map((b, idx) => (
                      <li key={idx} className="flex items-center justify-between bg-white/50 p-2 rounded-md">
                        <div><span className="font-semibold">{b.full_name}</span> - {b.date_of_birth}</div>
                        <div className="flex gap-2">
                          <button onClick={() => { setTemplate('birthday'); handleSendMessage(b.phone_number, b.full_name, 'whatsapp'); }} className="p-1 text-green-600 hover:bg-green-100 rounded-full" title="WhatsApp Birthday Wish"><MessageCircle size={18}/></button>
                          <button onClick={() => { setTemplate('birthday'); handleSendMessage(b.phone_number, b.full_name, 'sms'); }} className="p-1 text-blue-600 hover:bg-blue-100 rounded-full" title="SMS Birthday Wish"><MessageSquare size={18}/></button>
                        </div>
                      </li>
                    ))}
                  </ul>
          ...
          <div className="max-w-6xl mx-auto bg-white p-6 rounded-xl shadow-sm border">
            ...
            <div className="overflow-x-auto border border-gray-200 rounded-lg">
                <table className="w-full text-left border-collapse">
                    <thead><tr className="bg-gray-50 border-b border-gray-200 text-sm font-semibold text-gray-700"><th className="p-4">Name</th><th className="p-4">Status</th><th className="p-4">Phone</th><th className="p-4">Send Message</th></tr></thead>
                                ...
                                <tr key={idx} className="hover:bg-orange-50">
                                    ...
                                    <td className="p-4">
                                        <div className="flex items-center gap-3">
                                            <button onClick={() => handleSendMessage(guest.phone_number, guest.full_name, 'whatsapp')} className="text-green-600 hover:bg-green-100 p-2 rounded-full transition-colors" title="Send WhatsApp"><MessageCircle size={20} /></button>
                                            <button onClick={() => handleSendMessage(guest.phone_number, guest.full_name, 'sms')} className="text-blue-600 hover:bg-blue-100 p-2 rounded-full transition-colors" title="Send SMS"><MessageSquare size={20} /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))
        ...
        {activeView === 'absentees' && isAdmin && (
          <div className="max-w-6xl mx-auto bg-white p-6 rounded-xl shadow-sm border">
            ...
            <div className="overflow-x-auto border border-red-200 rounded-lg">
                <table className="w-full text-left border-collapse">
                    <thead><tr className="bg-red-50 border-b border-red-200 text-sm font-semibold text-red-800"><th className="p-4">Name</th><th className="p-4">Phone Number</th><th className="p-4">Send Message</th><th className="p-4">AI Drafter</th></tr></thead>
                    <tbody className="divide-y divide-gray-200">
                        {absenteeList.length === 0 ? (<tr><td colSpan={4} className="p-6 text-center text-gray-500 font-medium">Amazing! Everyone is accounted for.</td></tr>) : (
                            absenteeList.map((person, idx) => (
                                <tr key={idx} className="hover:bg-red-50 transition-colors">
                                    ...
                                    <td className="p-4">
                                        <div className="flex items-center gap-3">
                                            <button onClick={() => handleSendMessage(person.phone_number, person.full_name, 'whatsapp')} className="text-green-600 hover:bg-green-100 p-2 rounded-full transition-colors" title="Send WhatsApp"><MessageCircle size={20} /></button>
                                            <button onClick={() => handleSendMessage(person.phone_number, person.full_name, 'sms')} className="text-blue-600 hover:bg-blue-100 p-2 rounded-full transition-colors" title="Send SMS"><MessageSquare size={20} /></button>
                                        </div>
                                    </td>
                                    <td className="p-4"><button onClick={() => {setActiveView('outreach'); setPromptContext(`Draft a warm, caring "we miss you" message to ${person.full_name} who hasn't been to church in a couple of weeks.`);}} className="text-xs bg-white border border-red-300 text-red-600 px-3 py-1 rounded hover:bg-red-600 hover:text-white transition-colors">Custom AI Draft</button></td>
                                </tr>
                            ))
                        )}
                    </tbody>
